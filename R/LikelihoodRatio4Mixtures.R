...
#' @param NullMixture A Matrix: cbind(Means0,SDs0,Weights0) or cbind(Means0,SDs0,Weights0,IsLog0). The null model; usually with less Gaussians than the OneMixture 
#' @param OneMixture A Matrix: cbind(Means1,SDs1,Weights1) or cbind(Means1,SDs1,Weights1,IsLog1). The alternative model usually with more Gaussians than the OneMixture. 
#' @param PlotIt Optional: zero or one. o a Plot of the compared cdf's and the KS-test distribution (Diff)
#' @param LowerLimit Optional: test only for Data >= LowerLimit, Default = min(Data) i.e all Data.
...
#'   Vals=AdaptGauss(data2,c(-1,0,2),c(2,1,1),c(0.25,0.25,0.5),0.3,-6,6)
#'   NullMixture=cbind(Vals$Means,Vals$SDs,Vals$Weights)
#' }
#' 
#' \dontrun{
#'      Vals2=AdaptGauss(data2,c(-1,0,2,3),c(2,1,1,1),c(0.25,0.25,0.25,0.25),0.3,-6,6)
